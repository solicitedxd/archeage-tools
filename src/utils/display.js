...
((win) => {
  ...
  const check = () => {
    ...
    for (let elements, i = 0, len = listeners.length, listener; i < len; i++) {
      listener = listeners[i];
      // Query for elements matching the specified selector
      elements = doc.querySelectorAll(listener.selector);
      for (let element, j = 0, jLen = elements.length; j < jLen; j++) {
        element = elements[j];
