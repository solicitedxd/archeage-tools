...
export default ({ type, count, style }) => {
  if (type === REWARD.COIN) {
    ...
    return <Typography style={style}>{gold > 0 && <span className="currency gold">{gold}</span>}{silver > 0 &&
    <span className="currency silver">{silver}</span>}{copper > 0 &&
    <span className="currency copper">{copper}</span>}</Typography>;
  } else {
